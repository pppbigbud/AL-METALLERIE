 ...
 * 
 * NOTE: La metabox admin a été désactivée. Utiliser le bouton "Générer la description SEO"
 * dans la metabox existante (social-auto-publish.php) qui génère du contenu varié à chaque fois.
 * 
...
function almetal_generate_section_details($data) {
    ...
    $html .= '<div class="detail-item"><span class="detail-label">Matériau principal</span><span class="detail-value">' . $data['matiere_label'] . '</span></div>';
    $html .= '<div class="detail-item"><span class="detail-label">Finition</span><span class="detail-value">' . $data['peinture_label'] . '</span></div>';
    
    if (!empty($data['dimensions'])) {
        $html .= '<div class="detail-item"><span class="detail-label">Dimensions</span><span class="detail-value">' . esc_html($data['dimensions']) . '</span></div>';
    }
    if (!empty($data['poids'])) {
        $html .= '<div class="detail-item"><span class="detail-label">Poids</span><span class="detail-value">' . esc_html($data['poids']) . '</span></div>';
    }
    
    $html .= '<div class="detail-item"><span class="detail-label">Durée fabrication</span><span class="detail-value">' . esc_html($data['duree']) . '</span></div>';
    $html .= '<div class="detail-item"><span class="detail-label">Pose incluse</span><span class="detail-value">' . ($data['pose'] === 'oui' ? 'Oui' : 'Non') . '</span></div>';
    $html .= '<div class="detail-item"><span class="detail-label">Garantie</span><span class="detail-value">' . esc_html($data['garantie']) . '</span></div>';
    $html .= '<div class="detail-item"><span class="detail-label">Lieu d\'installation</span><span class="detail-value">' . esc_html($data['lieu']) . '</span></div>';
...
function almetal_generate_section_resultat($data) {
    ...
    $html .= '<div class="section-content"><p>' . $resultat . '</p>';
    
    if (!empty($data['temoignage'])) {
        $html .= '<blockquote class="client-testimonial"><p>"' . esc_html($data['temoignage']) . '"</p>';
...
function almetal_generate_section_similaires($data) {
    ...
    while ($similaires->have_posts()) {
        ...
        $html .= '<div class="similaire-content"><h3>' . get_the_title() . '</h3>';
        if ($lieu_sim) {
...
function almetal_schema_faq_realisation() {
    ...
    $faqs_by_type = array(
        'portails' => array(
            array('question' => 'Quel est le prix d\'un portail sur mesure ?', 'answer' => 'Le prix d\'un portail sur mesure varie entre 1 500€ et 5 000€ selon les dimensions, le matériau et la motorisation. Contactez-nous pour un devis personnalisé gratuit.'),
            array('question' => 'Quel délai pour la fabrication d\'un portail ?', 'answer' => 'Comptez en moyenne 2 à 4 semaines pour la fabrication d\'un portail sur mesure, selon la complexité du design et les finitions choisies.'),
            array('question' => 'Proposez-vous la motorisation des portails ?', 'answer' => 'Oui, nous proposons l\'installation de motorisations pour portails battants et coulissants, avec télécommande et options de domotique.')
        ),
        'garde-corps' => array(
            ...
            array('question' => 'Quel entretien pour un garde-corps en acier ?', 'answer' => 'Un garde-corps thermolaqué nécessite peu d\'entretien : un nettoyage à l\'eau savonneuse 2 fois par an suffit. La garantie anticorrosion est de 10 ans.')
        ),
        'escaliers' => array(
        ...
        'default' => array(
            ...
            array('question' => 'Quelles garanties offrez-vous ?', 'answer' => 'Nous offrons une garantie décennale sur la structure et 10 ans sur les finitions thermolaquées. Tous nos ouvrages sont conformes aux normes en vigueur.')
        )
    );
    ...
    $schema = array('@context' => 'https://schema.org', '@type' => 'FAQPage', 'mainEntity' => $faq_items);
...
function almetal_schema_product_realisation() {
    ...
    $schema = array(
        ...
        'image' => $image ?: '',
        'brand' => array('@type' => 'Brand', 'name' => 'AL Métallerie & Soudure'),
        'manufacturer' => array(
            '@type' => 'Organization',
            'name' => 'AL Métallerie & Soudure',
            'address' => array('@type' => 'PostalAddress', 'addressLocality' => 'Peschadoires', 'postalCode' => '63920', 'addressCountry' => 'FR')
        ),
